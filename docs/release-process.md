...
display name as committer. Two later v0.4.1 protected squash commits preserved
the same approved public author email while GitHub substituted the profile
display name and web-flow committer. Publication hygiene accepts these 29
immutable history exceptions only when the SHA-256 digest of each exact raw
commit object matches the 29-value allowlist in
`tests/publication_hygiene_tests.py`. The
...
## v0.4.1 fixed-worker stderr-drain repair

Birdsgone G8 operation `c9334675-1453-4b5c-9fe0-d301f95c33d3` returned
`GUEST_WORKER_FAILED`, `ok: false`, and `changed: false` after the repaired
input-binding build reached the real fixed worker. Read-only diagnosis proved
that Windows PowerShell 5.1 could write non-UTF-8 progress CLIXML to stderr.
The supervisor used a strict UTF-8 `StreamReader` for that non-result channel,
so its asynchronous read faulted; it also buffered the whole stream before a
post-hoc 64 KiB check.

The compatible repair leaves stdout as the sole strict UTF-8 JSON result
channel. It drains stderr as raw bytes through a fixed 4 KiB buffer, saturates
the recorded count at 64 KiB, continues draining and discarding after overflow,
and never decodes, accumulates, persists, logs, or exposes stderr content beyond
the transient fixed buffer. A bounded overflow maps to
`GUEST_WORKER_DIAGNOSTIC_TOO_LARGE`; all existing operation, invocation,
...
`0.4.1+codex.20260821104322`; do not regenerate it during review, merge,
installation, or the resumed G8 gate. Complete the affected Gate 2/Gate 7,
install-source, documentation, public-release, and exact-staged review gates.
Publish and merge only through the ordinary protected pull-request path, then
install that exact protected commit once through `scripts/install_plugin.ps1`.
Only a fresh selected-plugin task may reprove exactly 20 typed tools and resume
Birdsgone G8. No tag or GitHub Release is authorized.

The public-history validator binds the immediately preceding protected
input-binding squash commit to raw commit-object SHA-256

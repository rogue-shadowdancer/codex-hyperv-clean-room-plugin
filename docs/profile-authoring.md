...
fixture 位于 [`contracts/v2`](../contracts/v2/README.md)。Gate 7/H2 已把七个 schema
及其 production reader 集成到可安装 source；reader 按精确整数 `schemaVersion`
分派，不允许“先试 v2、失败后退回 v1”。未知版本返回
...
`MIGRATION_AMBIGUOUS_PACKAGE_KIND`，由作者补充信息。有效的 migrated
legacy profile 经过独立 v2 校验后复用保留的 v1 lifecycle runner，并继续
产生 v1 evidence；runtime 不得伪造只有 portable/driver/UI 自动化才能提供的

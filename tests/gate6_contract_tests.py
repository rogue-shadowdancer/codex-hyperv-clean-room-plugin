...
def assert_contract_metadata(catalog: dict[str, Any]) -> None:
    ...
    if catalog.get("currentRuntimeVersion") != "0.3.2":
        raise AssertionError("the executable patch runtime must be 0.3.2")
    ...
    if not re.fullmatch(
        ...
        raise AssertionError(
            "the integrated manifest must expose base 0.3.2 with at most "
...
def assert_v1_compatibility(catalog: dict[str, Any]) -> tuple[int, int]:
    ...
    if (
        ...
        or compatibility.get("currentRuntimeVersion") != "0.3.2"
    ):

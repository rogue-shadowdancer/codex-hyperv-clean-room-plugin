...
$sourceResult = & (Join-Path $PSScriptRoot 'validate-install-source.ps1') `
    -RequireCachebuster | ConvertFrom-Json -ErrorAction Stop
...
Assert-Gate4Ci ([int]$sourceResult.fileCount -eq 20 -and
    [int]$sourceResult.schemaCount -eq 5) `
    'Gate 4 source inventory no longer has 20 payload files and five schemas.'

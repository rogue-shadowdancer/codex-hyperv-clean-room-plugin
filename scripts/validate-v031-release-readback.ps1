...
$currentRelease = Invoke-HcrGhApi "repos/$Repository/releases/tags/v0.3.1"
Assert-HcrReadback ([string]$currentRelease.tag_name -ceq 'v0.3.1') `
    'The v0.3.1 Release tag is incorrect.'
Assert-HcrReadback ([string]$currentRelease.name -ceq 'v0.3.1') `
    'The v0.3.1 Release name is incorrect.'
Assert-HcrReadback ([string]$currentRelease.target_commitish -ceq $masterCommit) `
    'The v0.3.1 Release target does not equal protected master.'
Assert-HcrReadback (-not [bool]$currentRelease.draft -and
        -not [bool]$currentRelease.prerelease) `
    'The v0.3.1 Release is draft or prerelease.'
Assert-HcrReadback (-not [string]::IsNullOrWhiteSpace(
        [string]$currentRelease.published_at)) `
    'The v0.3.1 Release is not published.'
Assert-HcrReadback (@($currentRelease.assets).Count -eq 0) `
    'The v0.3.1 source-only Release has uploaded assets.'

...
$requiredPhrases = [ordered]@{
    ...
    'README.md' = @('v0.1.1', 'GPL-3.0-only', '16 MCP tools', '20 MCP tools', '0.2.0', 'notPerformed')
    'TASK_HANDOFF.md' = @('relayProtocolVersion', 'validate-gate4.ps1', 'H4/G9', 'notPerformed')
    'docs\architecture.md' = @('PowerShell Direct', 'trust boundar', 'operation-scoped', '16', 'Schema-v2 integrated architecture', 'paired recovery')
    'docs\installation.md' = @('install_plugin.ps1', 'check_install.ps1', 'ownership', 'marketplaceVisible', 'SHA-256', '20 tools')
    ...
    'docs\specification.md' = @('Plugin 0.2.0 and schema-v2 target contract', 'plan_vm_power', 'plan_vm_network', 'portable-manifest.json', 'data-testid', 'UNSUPPORTED_SCHEMA_VERSION', 'Gate 6/H1 acceptance boundary', 'Gate 9/H4 acceptance boundary')
    'docs\profile-authoring.md' = @('Schema-v2 portable automation', 'portableAutomation', 'uiUploadFixture', 'MIGRATION_AMBIGUOUS_PACKAGE_KIND')
    'docs\evidence.md' = @('sourceSha256', 'guestSha256', 'cleanupTriggered', 'manual', 'machineStatus', 'overallStatus')
    'docs\security.md' = @('DPAPI', 'plaintext', 'reparse', 'arbitrary', 'WebDriver supply chain', 'Schema downgrade')
